...
{
  if (chars)
    *chars = gps->_encoded_characters;
  if (sentences)
    *sentences = gps->_good_sentences;
  if (failed_cs)
    *failed_cs = gps->_failed_checksum;
}
#endif
...
{
  if (latitude)
    *latitude = gps->_latitude;
  if (longitude)
    *longitude = gps->_longitude;
  if (fix_age)
    *fix_age = gps->_last_position_fix == GPS_INVALID_FIX_TIME ? GPS_INVALID_AGE : millis() - gps->_last_position_fix;
}
...
void get_datetime(TinyGPS *gps, unsigned long *date, unsigned long *time, unsigned long *fix_age)
{
  ...
  if (time)
    *time = gps->_time;
  if (fix_age)
    *fix_age = gps->_last_time_fix == GPS_INVALID_FIX_TIME ? GPS_INVALID_AGE : millis() - gps->_last_time_fix;
}
